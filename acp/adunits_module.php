...
class adunits_module
{
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			...
			$display_vars = array(
				'title'	=> 'ACP_ADUNITS',
				'vars'	=> array(
					'legend1'	=> 'ADUNITS_EXCLUDE',
					'adunits_exclude'		=> array('lang' => 'ACP_ADUNITS_EXCLUDE', 'validate' => 'string',	'type' => 'custom', 'method' => 'select_exclude_pages', 'explain' => true),
		...
		if ($submit)
		{
			if ($mode == 'exclude')
			{
				$excludes = $request->variable('adunits_exclude', array(0 => ''));
				$config->set('adunits_exclude', implode(',', $excludes));
			}
			else
			{
	...
	*/
	...
	function select_exclude_pages($value, $key)
	{
		global $config, $user;
		...
		$explain = array(
			...
			$user->lang['SEARCH'],// search
			$user->lang['FAQ'],
			$user->lang['MEMBERLIST'],
			$user->lang['POST_REPLY'],//posting
			$user->lang['REPORTING_POST'],//report
			$user->lang['VIEWING_ONLINE'], //viewonline
			$user->lang['FEED'],//feed
			$user->lang['MCP'],//mcp
			'',//app
			$user->lang['INDEX'],
		...
		$act_options = '<select id="' . $key . '" name="' . $key . '[]" multiple="multiple" size="10">';
		foreach ($pages_list as $k => $v)
		{
			$act_options .= '<option value="' . $k . '"' . ((in_array($k, $selected)) ? ' selected="selected"' : '') . '>' . $v . ' --> ' . $explain[$k] . '</option>';
		}
		$act_options .= '</select>';
